...
import DownloadOrdersTemplate from './DownloadOrdersTemplate';
...
const generatePdfDocument = async (data: any) => {
  const blob = await pdf(<DownloadOrdersTemplate data={data} />).toBlob();
  fileSaver.saveAs(blob, 'order-details.pdf');
};
...
export default function OrderHistoryItem({
  ...
  const [isLineCalcExpanded, setIsLineCalcExpanded] = useState<boolean>(true);
        ...
        <div className="flex flex-col">
          ...
          <div className="p-2 lg:py-3 lg:px-6 gap-2 lg:gap-6 grid grid-cols-2 sm:flex justify-end items-center">
            ...
            <Button
              onClick={() => {
                // window.open(
                //   `http://localhost:3000/order-export/${order?.code}`,
                // );
